...
library(scales)      # Scaling functions for plotting
library(purrr)       # List-column and functional helpers

# Read trigeminal sensitivity data from Excel file
trigem_data <- read_excel(
  "/home/joern/Aktuell/TrigeminalSensitivity/09Originale/Bormann Trigeminale Studie Daten.xlsx",
  sheet = "Tabelle1"
)

# Select variables related to nasal breathing problems
...
# Subset data for nasal breathing related variables
...
# Add age and current year variables for date calculations
nasal_breathing_df$age <- as.integer(trigem_data$Alter)
current_year <- year(Sys.Date())

# Function to robustly extract year(s) from raw text descriptions
extract_years_custom <- function(text, age, current_year = 2025) {
  ...
  # Special cases: approximate birth years based on age
  ...
  # Ongoing or periodic keywords
  if (str_detect(text, "regelmäßig|halbjährlich|Quartal|Kindesalter")) {
    return("ongoing")
  }

  # Plain 4-digit year if present
  if (str_detect(text, "^\\d{4}$")) {
    return(text)
  }

  # Return NA if no pattern matched
  return(NA_character_)
}

# Apply year extraction and infer nasal breathing problem flags
...
# Function to translate German therapy descriptions to English
translate_therapy <- function(therapy_text) {
  if (is.na(therapy_text)) return(NA_character_)
  match_key <- names(therapy_dict)[
    map_lgl(names(therapy_dict),
            ~ str_detect(therapy_text, fixed(.x, ignore_case = TRUE)))
  ]
...
# Translate therapies for all rows
nasal_breathing_df <- nasal_breathing_df %>%
  mutate(
...
# Prepare plot data for nasal breathing problems by case-year
plot_nasal_breathing <- nasal_breathing_df %>%
  filter(nasal_breathing_problem == 1) %>%
  mutate(
    years_expanded = ifelse(is.na(extracted_years), list(NA_character_), extracted_years)
  ) %>%
  unnest(years_expanded) %>%
  mutate(
    years_expanded = ifelse(years_expanded == "ongoing", NA_character_, years_expanded),
...
with_year_points <- plot_nasal_breathing %>% filter(!is.na(year_num)) %>% select(case_id, year_num)
without_year_points <- plot_nasal_breathing %>% filter(is.na(year_num) & actual_nasal_breathing_problem == 1) %>%
...
# Plot nasal breathing problems timeline
...
# Prepare therapy occurrence data plotting therapy (y) vs year (x)
...
# Final therapy plot with alternating striped background
nasal_breating_therapy_plot <- ggplot() +
  ...
  geom_jitter(data = df_therapy_long,
              ...
              width = 0.3, height = 0.3, alpha = 0.6, color = "black", show.legend = FALSE) +
  ...
  labs(x = "Therapy", y = "Year", title = "Therapies for nasal breathing problems over years") +
  theme_minimal(base_size = 8) +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
    panel.grid.major.x = element_blank(),
  ...
  geom_hline(yintercept = seq(min_year, max(df_therapy_long$year_plot, na.rm = TRUE)),
             color = "grey80", size = 0.3, linetype = "dashed")

print(nasal_breating_therapy_plot)

ggsave(paste0("p_nasal_breating_therapy_plot", ".svg"), nasal_breating_therapy_plot, width = 12, height = 12)


# Ensure therapy_english is a factor
nasal_breathing_df$therapy_english <- factor(nasal_breathing_df$therapy_english)

# Create one-hot encoded matrix (no intercept)

...
performProjection <-
  function( X,
    ...
    # Function to remove collinearity using regression-based VIF with fallback to correlation-based approach
    if ( remove_colinear_vars ){
      remove_collinear_with_fallback <- function( df, target_col, vif_threshold = 10, correlation_threshold = 0.9 ) {
        # Ensure the target column is present in the data frame
        ...
        # Helper function to remove collinear variables based on correlation
        ...
        # First try using VIF-based collinearity removal
        ...
        if ( !inherits( try_vif, "try-error" ) && ncol( try_vif ) > 0 ) {
          ...
          # Fallback to correlation-based collinearity removal
        ...
        # Combine the cleaned predictors with the target column to return the final data frame
    ...
    # #Remove colinear vars for ICA or PLSICA
    # if (correlation_threshold_for_colinearity < 1)   {
    #
    #   remove_collinear <- function(df, threshold = 0.9) {
    #     cor_matrix <- cor(df)
    ...
    #   }
    #
    #   correlation_threshold_for_colinearity <- min(0.01, correlation_threshold_for_colinearity)
    #   X_unique_corr_removed  <- remove_collinear(X_unique, threshold = correlation_threshold_for_colinearity)
    #   X_unique <- X_unique_corr_removed
    # }
    ...
    switch( projection_method,
            ...
            "ICA" = {
              # Helper function to sort ICA components for importance
            ...
            "tSNE" = {
              ...
              Proj <- Rtsne::Rtsne( X_unique, dims = min(ncomp, 3), perplexity = perpl )[[2]]
            },
            "isomap" = {
            ...
            "LDA" = {
              ...
              res.lda <- MASS::lda( factor( Target_unique ) ~ ., data = dfLDA, dimen = ncomp )
              lda_projection <- predict( res.lda, X_unique, dimen = ncomp )
            ...
            "LLE" = {
              ...
              #     local_var <- apply(local_diff, 1, stats::var)
              ...
              lle_optimized <- function( data, max_dim = 10, max_k = 30, perplexity = 30 ) {
                ...
                optimize_params <- function( X, max_k, max_d, perplexity ) {
                  ...
                  tsne_emb <- Rtsne::Rtsne( X, dims = min(ncomp, 3), perplexity = perplexity, verbose = FALSE )$Y
